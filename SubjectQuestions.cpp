...
void SubjectQuestions::addQuiz(const string subjectName) {
    ...
    int i = 1, choice;
    string cOption;
    bool found = false, validOption=false;
...
void SubjectQuestions::updateQuiz() {
    ...
    bool found = false, validOption,correctID=false;
    int selection;
    char choice;
    system("cls");
    subjectLocation = getSubjectFilePath();
...
void SubjectQuestions::deleteSubject() {
    ...
    while (getline(file, fileSubject)) {
        ...
        if (subject == fileSubject) {
            found = true;
            cout << "\n\t\t\t\t\tIs this Subject you want to delete? "<<subject;
            cin >> inp;
            inp=toupper(inp);
            if (inp=='Y'){
    ...
    if (found) {
        ...
        cout << "\n\n\\at\t\t\t\tSubeject Not Found. Nothing to delete" << endl;
    }
